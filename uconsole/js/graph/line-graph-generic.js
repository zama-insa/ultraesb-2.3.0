...
//            multiData[stream].reverse();
            ...
            if (graphs[stream].stats != null) {
                ...
                if (min != null) {
                    $('#' + min).html(formatDecimalNumberWithMinify(dAll.min));
                }
                var max = graphs[stream].stats.max;
                if (max != null) {
                    $('#' + max).html(formatDecimalNumberWithMinify(dAll.max));
                }
                var count = graphs[stream].stats.count;
                if (count != null) {
                    $('#' + count).html(formatNumberWithMinify(dAll.count));
                }
                var sum = graphs[stream].stats.sum;
                if (sum != null) {
                    $('#' + sum).html(formatNumberWithMinify(dAll.sum));
                }
                var rate = graphs[stream].stats.rate;
                if (rate != null) {
                    $('#' + rate).html(formatDecimalNumberWithMinify(dAll.averageRate));
                }
