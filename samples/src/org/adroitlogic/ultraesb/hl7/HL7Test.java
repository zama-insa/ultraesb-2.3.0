 ...
 * @author asankha
 */
public class HL7Test extends UTestCase {
    ...
    public static Test suite() {
        return new TestSetup(new TestSuite(HL7Test.class)) {
    ...
    public void testAckOverMLLP() throws Exception {

        Parser p = new GenericParser();
        Message adt = p.parse(msg);

        MyConnectionHub cHub = MyConnectionHub.getInstance();
        Connection connection = cHub.attach("localhost", 2200, false, new PipeParser(), MinLowerLayerProtocol.class);
    ...
    public void testNackOverMLLPS() throws Exception {

        System.setProperty("javax.net.ssl.trustStore", "conf/keys/identity.jks");
        ...
        MyConnectionHub cHub = MyConnectionHub.getInstance();
        Connection connection = cHub.attach("localhost", 2300, true, new PipeParser(), MinLowerLayerProtocol.class);
        ...
        String responseString = parser.encode(response);
        logger.info("Received response:\n" + responseString);
        assertTrue(responseString.contains("My Error"));
        connection.close();
    }
}
