...
/**
 * Provides access to shared HL7 Connections.  The MyConnectionHub
 * has at most one connection to any given address at any time.
 * @author Bryan Tripp
 */
public class MyConnectionHub {

    private static final HapiLog log = HapiLogFactory.getHapiLog(MyConnectionHub.class);

    private static MyConnectionHub instance = null;
    private HashMap connections;
    private HashMap sockets;
    private HashMap numRefs;

    /** Creates a new instance of MyConnectionHub */
    private MyConnectionHub() {
        connections = new HashMap(20);
        sockets = new HashMap(20);
        numRefs = new HashMap(20);
    }

    /** Returns the singleton instance of MyConnectionHub */
    public synchronized static MyConnectionHub getInstance() {
        if (instance == null) {
            instance = new MyConnectionHub();
        }
        return instance;
    }
    ...
    public Connection attach(String host, int port, Parser parser, Class llpClass) throws HL7Exception {
        return attach(host, port, false, parser, llpClass);
    }
     ...
     * is using, call <code>Connection.getParser()</code>.
     */
    public Connection attach(String host, int port, boolean ssl, Parser parser, Class llpClass) throws HL7Exception {
        ...
        if (conn == null) {
            try {
                ...
                conn = connect(host, port, ssl, parser, llp);
     ...
     * connections Hash.
     */
    private Connection connect(String host, int port, boolean ssl, Parser parser, LowerLayerProtocol llp) throws UnknownHostException, IOException, LLPException {
        Socket s = null;
        if (ssl) {
    ...
    /**
     * Informs the MyConnectionHub that you are done with the given Connection -
